...
fun AppUpdateItem(
    ...
    // בדיקה האם האפליקציה מפוטצ'ת והאם הפאצ' החדש עדיין בהכנה
    val isPatched = com.aurora.gplayapi.PatchedAppManager.isPatchedApp(update.packageName)
    val installedVersionName = androidx.compose.runtime.remember(update.packageName) {
        com.aurora.store.util.PackageUtil.getInstalledVersionName(context, update.packageName)
    }
    val isPatchedPending = isPatched && !com.aurora.gplayapi.PatchedAppManager.isPatchedUpdateReady(update.packageName, installedVersionName)
    ...
    Row(
        ...
        when {
            ...
            // --- 🚀 אם הפאצ' החדש עדיין בהכנה: הצג כפתור מנוטרל ---
            isPatchedPending -> {
                OutlinedButton(onClick = {}, enabled = false) {
                    Text("פאצ' בהכנה")
                }
